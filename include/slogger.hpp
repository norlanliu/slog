...
void InitPath(const char* subsystem_name, const char* path,  const char* sys_log_name = NULL, const char* debug_log_name = NULL) {
...
void InitGlobalAttributes() {
	...
	core->add_global_attribute(MODULE_FLAG_STRING, attrs::constant < std::string > (g_subsystem_name));
}
...
void InitSinks() {
	...
	boost::shared_ptr< sinks::text_file_backend > system_text_backend =
		boost::make_shared< sinks::text_file_backend >(
				keywords::file_name = system_path,
				keywords::auto_flush = true,
				keywords::open_mode = (std::ios::out | std::ios::app)
				);
	...
	multiple_file_backend->set_file_name_composer(
			sinks::file::as_file_name_composer(
					expr::stream << log_path << expr::attr < std::string > (FILE_ID_FLAG_STRING) << ".log"));
...
Logger SetLogName(std::string file_name) {
	Logger multi_logger = boost::make_shared< src::severity_logger_mt<SeverityLevel> >();
	multi_logger->add_attribute(log_inner::FILE_ID_FLAG_STRING,
			attrs::constant < std::string > (file_name));
	multi_logger->add_attribute(log_inner::TYPE_FLAG_STRING, attrs::constant<int>(10));
	return multi_logger;
}
